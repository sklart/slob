...
** Command line interface
   ...
   #+BEGIN_SRC
   usage: slob [-h] {find,aliases,get,info,verify,tag,convert,merge} ...

   positional arguments:
     {find,aliases,get,info,verify,tag,convert,merge}
       find               Find keys
       aliases            Find blob aliases
       get                Retrieve blob content
       info               Inspect slob and print basic information about it
       verify             Validate SLOB structure and payloads
       tag                List tags, view or edit tag value
       convert            Re-encode, re-compress or split a SLOB
       merge              Repack multiple SLOB files into one SLOB
   ...
   To validate the structure or read every stored blob:
   #+BEGIN_SRC sh
   slob verify my.slob
   slob verify --full my.slob
   #+END_SRC

   To combine volumes (see the merge section below for details):
   #+BEGIN_SRC sh
   slob merge combined.slob volume-01.slob volume-02.slob
   #+END_SRC
...
** Merge SLOB volumes
   ...
   =slob merge= repacks several ordinary SLOB files into one compatible
   SLOB.  It is not a binary concatenation: every referenced blob is read and
   written again using the selected output compression.  Keys, aliases,
   fragments and content types are preserved; duplicate keys from different
   inputs are also preserved.  Content is not deduplicated across inputs.

   #+BEGIN_SRC sh
   slob merge --compression lzma2 --verify full ruwiki-merged.slob \
     ruwiki-0000001-0010000.slob ruwiki-0010001-0020000.slob
   #+END_SRC

   The merge is atomic: it writes a temporary sibling of the destination and
   replaces the destination only after finalization and verification.  An
   existing output is never overwritten.  Input encodings must match.  User
   tags are taken from the first input by default; use =--tag-policy common=
   or =--tag-policy error= to choose a stricter policy.
